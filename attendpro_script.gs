...
// ── ADD RECORD ───────────────────────────────────────────────────
// Hoja "Registros" — una fila por evento (entrada o salida)
// Col A: Nombre
// Col B: Área/Rol
// Col C: Fecha (dd/MM/yyyy)
// Col D: Hora
// Col E: Tipo (Entrada / Salida)
// ─────────────────────────────────────────────────────────────────
function handleAddRecord(params) {
  const sheet = getOrCreateSheet(SHEET_REGISTROS,
    ['Nombre', 'Área/Rol', 'Fecha', 'Hora', 'Tipo']);
  ...
  const tipo  = type === 'entrada' ? 'Entrada' : 'Salida';

  sheet.appendRow([name, role, fecha, hora, tipo]);

  // Formato zebra
  const lastRow = sheet.getLastRow();
  if (lastRow % 2 === 0) {
    sheet.getRange(lastRow, 1, 1, 5).setBackground('#f3f4f6');
  }
